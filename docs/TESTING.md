...
- Select Browser, save, and verify the QR and copied URL contain the sender's current Wi-Fi IPv4 address, live port `49373`, and a random access key.
...
- Start a native Receiver App broadcast and verify the same private browser link also loads while the broadcast is live.
